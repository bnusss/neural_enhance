...
add_arg('--nums',               default=-1, type=int,               help='How many images to train, -1 means ALL.')
...
add_arg('--pixel-weight',  	default=1e6, type=float,            help='Weight of the pixel loss.')
...
class DataLoader(threading.Thread):
    ...
    def __init__(self):
        ...
        self.files = self.files[:args.nums] if args.nums != -1 else self.files
        print('Train with {} images.'.format(len(self.files)))
...
class Model(object):
    ...
    def make_block(self, name, input, units):
        resBlock_input = input
        for i in range(args.generator_residual):
            resBlock_input = self.make_layer(name+'-BL/%i'%i, resBlock_input, units, alpha=0.5) 
    ...
    def loss_pixel(self, y, y_hat):
        return lasagne.objectives.squared_error(y, y_hat).mean()
    ...
    def compile(self):
        ...
        gen_losses = [self.loss_perceptual(percept_out) * args.perceptual_weight,
		      ...
		      self.loss_pixel(gen_out, input_tensor) * args.pixel_weight]
        ...
        try:
            ...
            for epoch in range(args.epochs):
                ...
                for _ in range(args.epoch_size):
                    ...
                    losses = np.array(output[:4], dtype=np.float32)
                    stats = (stats + output[4]) if stats is not None else output[3]
                ...
                totals, labels = [sum(total)] + list(total), ['total', 'prcpt', 'smthn', 'advrs', 'pixels']
                ...
                print('  - generator losses: {}'.format(' '.join(gen_info)))
